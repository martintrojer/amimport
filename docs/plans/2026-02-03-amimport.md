...
## Overview
Build a native macOS SwiftUI app (`AMImport`) with a format-extensible import pipeline (CSV first), that matches imported songs against the user’s Apple Music library using configurable strategies, lets the user manually resolve misses, and exports resolved tracks to either a new Music playlist or the current queue.
...
- [x] Task 16: Build unresolved match resolution UI
- [x] Task 17: Add manual library search service
- [x] Task 18: Define export contracts and DTOs
- [x] Task 19: Implement Music app bridge for playlist + queue
- [x] Task 20: Build export screen and execution flow
- [x] Task 21: Persist import sessions for resume support
- [x] Task 22: End-to-end happy path test with fakes
- [x] Task 23: Document setup, constraints, and troubleshooting
